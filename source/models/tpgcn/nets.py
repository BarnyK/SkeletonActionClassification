...
class ResGCN_Input_Branch(nn.Module):
    def __init__(self, structure, spatial_block, temporal_block, num_channel, A, **kwargs):
        ...
        module_list += [ResGCN_Module(64, 64, 'Basic', 'Basic', A, initial=True, **kwargs) for _ in
                        range(structure[0] - 1)]
        module_list += [ResGCN_Module(64, 64, spatial_block, temporal_block, A, **kwargs) for _ in
                        range(structure[1] - 1)]
